...
import { SUSPENSION, WHEEL_LAYOUT } from './VehicleTuning';
...
export class Suspension {
  ...
  private readonly rayLength = SUSPENSION.restLength + SUSPENSION.wheelRadius + SUSPENSION.raySkin;
  ...
  constructor() {
    for (let i = 0; i < WHEEL_LAYOUT.length; i++) {
      ...
      this.wheels.push({
        ...
        isFront: l.front,
        isLeft: l.left,
        anchorLocal: new THREE.Vector3(l.x, SUSPENSION.anchorY, l.z),
        anchorWorld: new THREE.Vector3(),
        grounded: false,
        compression: 0,
        visualCompression: 0,
        springLength: SUSPENSION.restLength,
  ...
  reset(): void {
    for (let i = 0; i < this.wheels.length; i++) {
      ...
      w.springLength = SUSPENSION.restLength;
      w.load = 0;
      w.slip = 0;
      w.slipLat = 0;
      w.slipLong = 0;
  ...
  step(
    ...
    for (let i = 0; i < this.wheels.length; i++) {
      ...
      if (!usable || hit === null) {
        ...
        w.centerWorld
          .copy(dir)
          .multiplyScalar(SUSPENSION.restLength)
          .add(w.anchorWorld);
        w.contactPoint
          .copy(dir)
          .multiplyScalar(SUSPENSION.restLength + SUSPENSION.wheelRadius)
          .add(w.anchorWorld);
      ...
      /* --- geometry --- */
      const centreDist = hit.distance - SUSPENSION.wheelRadius;
      if (centreDist > SUSPENSION.restLength) {
      ...
      const springLength = clamp(centreDist, 0, SUSPENSION.restLength);
      const compression = clamp01(1 - springLength / SUSPENSION.restLength);
      ...
      const rate = w.isFront ? SUSPENSION.springRateFront : SUSPENSION.springRateRear;
      const springForce = rate * compression * SUSPENSION.restLength;

      const vNormal = this.tmpVel.dot(w.contactNormal);
      const damperRate =
        vNormal < 0 ? SUSPENSION.damperCompress : SUSPENSION.damperRebound;
      const damperForce = -damperRate * vNormal;

      const force = clamp(springForce + damperForce, 0, SUSPENSION.maxSpringForce);
      w.load = force;
      total += force;
    ...
    this.applyAntiRoll(body, frame, dt, 0, 1, SUSPENSION.antiRollFront);
    this.applyAntiRoll(body, frame, dt, 2, 3, SUSPENSION.antiRollRear);
  }
  ...
  private applyAntiRoll(
    ...
    const force = clamp(diff * rate, -SUSPENSION.maxSpringForce * 0.5, SUSPENSION.maxSpringForce * 0.5);
  ...
  updateVisual(dt: number): void {
    for (let i = 0; i < this.wheels.length; i++) {
      const w = this.wheels[i];
      const target = w.grounded ? w.compression : 1 - SUSPENSION.airborneDroop;
      w.visualCompression = damp(
        w.visualCompression,
        target,
        SUSPENSION.visualSmoothRate,
        dt,
      );
    }
  }
